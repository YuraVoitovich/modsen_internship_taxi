...
interface RideRepository: JpaRepository<Ride, UUID> {
    @Query(
      value = "select id, passenger_profile_id as passengerProfileId, start_geo as startGeo, end_geo as endGeo, ST_Distance(geography(start_geo), end_geo) as distance from ride where st_dwithin(geography(start_geo), :point, :radius)",
      nativeQuery = true
    )
